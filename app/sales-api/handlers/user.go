...
func (ug userGroup) query(ctx context.Context, w http.ResponseWriter, r *http.Request) error {
	ctx, span := otel.Tracer("service").Start(ctx, "handlers.user.query")
	defer span.End()
	...
	if err != nil {
		return web.NewRequestError(
			fmt.Errorf("invalid page format: %s",
				params["page"]), http.StatusBadRequest,
		)
	}
	rowsPerPage, err := strconv.Atoi(params["rows"])
	if err != nil {
		return web.NewRequestError(
			fmt.Errorf("ivalid rows format: %s",
				params["rows"]), http.StatusBadRequest,
		)
	}
...
func (ug userGroup) queryByID(ctx context.Context, w http.ResponseWriter, r *http.Request) error {
	ctx, span := otel.Tracer("service").Start(ctx, "handlers.user.queryByID")
	defer span.End()
...
func (ug userGroup) create(ctx context.Context, w http.ResponseWriter, r *http.Request) error {
	ctx, span := otel.Tracer("service").Start(ctx, "handlers.user.create")
	defer span.End()
...
func (ug userGroup) update(ctx context.Context, w http.ResponseWriter, r *http.Request) error {
	ctx, span := otel.Tracer("service").Start(ctx, "handlers.user.update")
	defer span.End()
...
func (ug userGroup) delete(ctx context.Context, w http.ResponseWriter, r *http.Request) error {
	ctx, span := otel.Tracer("service").Start(ctx, "handlers.user.delete")
	defer span.End()
...
func (ug userGroup) token(ctx context.Context, w http.ResponseWriter, r *http.Request) error {
	ctx, span := otel.Tracer("service").Start(ctx, "handlers.user.token")
	defer span.End()
	v, ok := ctx.Value(web.KeyValues).(*web.Values)
	if !ok {
		return web.NewShutdownError("web value missing from context")
	...
	token.Token, err = ug.auth.GenerateToken(params["kid"], claims)
	if err != nil {
