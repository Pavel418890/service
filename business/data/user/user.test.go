...
func TestUser(t *testing.T) {
	...
	t.Log("Given the need to work with User records.")
	{
		...
		{
			ctx := tests.Context()
			now := time.Date(2022, time.December, 1, 0, 0, 0, 0, time.UTC)
			traceID := "00000000-0000-0000-0000-000000000000"

			nu := user.NewUser{
				Name:            "Pavel Lots",
				Email:           "pavel418890@gmail.com",
				Roles:           []string(auth.RoleAdmin),
				Password:        "gothers",
				PasswordConfirm: "gotherps",
			}
		...
		t.Logf("\t%s\tTest %d:\tShould be able to update user.", test.Success, testID)
		...
		t.Logf("\t%s\tTest %d:\tShould be able to retrieve user.", test.Success, testID)
		...
		if err := u.Delete(ctx, db, u.ID); err != nil {
