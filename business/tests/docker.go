...
// Container tracks infomation about the docker container started for tests.
...
func extractIPPort(t *testing.T, doc []map[string]interface{}, port string) (string, string) {
	...
	if !exists {
		t.Fatal("could not get network settings key")
	}
	ports, exists := nw.(map[string]interface{})["Ports"]
	if !exists {
		t.Fatal("could not get ports settings key")
	}
